package ramizbek.aliyev.editcolorfragment

import android.os.Bundle
import androidx.fragment.app.Fragment
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import ramizbek.aliyev.editcolorfragment.databinding.FragmentBlank2Binding
import ramizbek.aliyev.editcolorfragment.databinding.FragmentBlankBinding

// TODO: Rename parameter arguments, choose names that match
// the fragment initialization parameters, e.g. ARG_ITEM_NUMBER
private const val ARG_PARAM1 = "param1"
private const val ARG_PARAM2 = "param2"
lateinit var binding: FragmentBlank2Binding

/**
 * A simple [Fragment] subclass.
 * Use the [BlankFragment2.newInstance] factory method to
 * create an instance of this fragment.
 */
class BlankFragment2 : Fragment() {
    // TODO: Rename and change types of parameters
    private var param1: String? = null
    private var param2: String? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        arguments?.let {
            param1 = it.getString(ARG_PARAM1)
            param2 = it.getString(ARG_PARAM2)
        }
    }

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentBlank2Binding.inflate(layoutInflater)
        val blankFragmentBlank = BlankFragment()
        binding.apply {
            view1.setOnClickListener {
                User.color = view1.background
            }
            view2.setOnClickListener {
                User.color = view2.background
            }
            view3.setOnClickListener {
                User.color = view3.background
            }
            view4.setOnClickListener {
                User.color = view4.background
            }
            view5.setOnClickListener {
                User.color = view5.background
            }
            view6.setOnClickListener {
                User.color = view6.background
            }
            view7.setOnClickListener {
                User.color = view7.background
            }
            view8.setOnClickListener {
                User.color = view8.background
            }
            view9.setOnClickListener {
                User.color = view9.background
            }
            view10.setOnClickListener {
                User.color = view10.background
            }
            view11.setOnClickListener {
                User.color = view11.background
            }
            view12.setOnClickListener {
                User.color = view12.background
            }

            binding.btnSetColor.setOnClickListener {
                val fragmentManager = parentFragmentManager
                val transaction = fragmentManager.beginTransaction()
                transaction.replace(R.id.myContainer, blankFragmentBlank)
                transaction.commit()
            }
        }
        return binding.root
    }

    companion object {
        /**
         * Use this factory method to create a new instance of
         * this fragment using the provided parameters.
         *
         * @param param1 Parameter 1.
         * @param param2 Parameter 2.
         * @return A new instance of fragment BlankFragment2.
         */
        // TODO: Rename and change types and number of parameters
        @JvmStatic
        fun newInstance(param1: String, param2: String) =
            BlankFragment2().apply {
                arguments = Bundle().apply {
                    putString(ARG_PARAM1, param1)
                    putString(ARG_PARAM2, param2)
                }
            }
    }
}